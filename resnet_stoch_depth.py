...
def _bn_relu_conv(**conv_params):
    ...
    def f(input):
        ...
        # activation = SpatialDropout2D(0.1)(activation)
...
def _shortcut(input, residual):
    ...
    # y = BatchNormalization(axis=3)(y), y>residual
    global num_reps
    global rep_iter
    block, nb_total_blocks = rep_iter, num_reps
    p_survival = get_p_survival(block=block, nb_total_blocks=nb_total_blocks, p_survival_end=0.5, mode='linear_decay')
    print(p_survival)
    residual = Lambda(stochastic_survival, arguments={'p_survival': p_survival})(residual)
...
class ResnetBuilder(object):
    ...
    def build(input_shape, num_outputs, block_fn, repetitions):
        ...
        model = Model(inputs=input, outputs=dense_soft)
        return model
    ...
    @staticmethod
    def build_resnet_stoch(input_shape, num_outputs):
        return ResnetBuilder.build(input_shape, num_outputs, bottleneck, [3, 4, 6, 8, 12, 3])
