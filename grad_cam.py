...
def compile_saliency_function(model, activation_layer='block5_conv3'):
    input_img = model.get_input_at(0)# input
...
def grad_cam(input_model, image, category_index, layer_name):
    ...
    gradient_function = K.function([model.layers[0].get_input_at(0)], [conv_output, grads])#get_input_at(node_index) input
    ...
    image = image[0, :]
    image -= np.min(image)
    image = np.minimum(image*255, 255)
