...
from utils_run import ld_obj, save_obj, bothIms_and_Demo, perfMeasures, weighted_categorical_crossentropy, hist_type, restrict
import numpy as np
import csv
import os
...
def getInps(spl = 0.3, Topt = np.array([1])):
	...
	print('train')
	trnIms, trnLs, trnR= restrict(csv_name, trnIms, trnLs)
	print('test')
	tstIms, tstLs, tstR= restrict(csv_name, tstIms, tstLs)
	print('val')
	valIms, valLs, valR= restrict(csv_name, valIms, valLs)
	...
	Rs = np.array(trnR+tstR+valR)
	RsIms, RsLs  = Rs[:, :4], Rs[:, 4]
	return trnLs, trnIms, tstLs, tstIms, valLs, valIms, tstLsI, tstImsI, tstLsB, tstImsB, RsLs, RsIms

trnLs, trnIms, tstLs, tstIms, valLs, valIms, tstLsI, tstImsI, tstLsB, tstImsB, RsLs, RsIms = getInps()
...
RsIms, RsDs = resize_to_224_images(RsIms)
...
T1_tst_R, T2_tst_R, y_tst_R = RsIms[:,:,:,:,0], RsIms[:,:,:,:,1], np_utils.to_categorical(RsLs, nb_classes)


mn, s = -1,-1
def sampleWiseNorm(ims):
    return np.stack([(im-np.mean(im))/np.std(im) for im in ims],axis=0)
T1_train, T2_train, T1_val, T2_val, T1_tst, T2_tst, T1_tst_I, T2_tst_I, T1_tst_B, T2_tst_B, T1_tst_R, T2_tst_R = list(map(sampleWiseNorm, [T1_train, T2_train, T1_val, T2_val, T1_tst, T2_tst, T1_tst_I, T2_tst_I, T1_tst_B, T2_tst_B, T1_tst_R, T2_tst_R]))
...
x_tst_R = [T1_tst_R, T2_tst_R, np.stack(RsDs,axis=0)]
...
for rep in range(5):
	for kr in [0.05,0.07,0.1,0.15,0.2]:
		for ar in [0.007,0.01,0.013,0.016,0.02]:
			...
			evl_tst_R = model.evaluate(x=x_tst_R, y=y_tst_R)
			print('testR: '+str(evl_tst_R))
			...
			pred1_ts_R = model.predict(x_tst_R)
			nm = fl_nm
			tp = 'multi'
			...
			for p, y, trnvaltst in zip([pred1_tr, pred1_vl, pred1_ts, pred1_ts_I, pred1_ts_B, pred1_ts_R],[y_train, y_val, y_tst, y_tst_I, y_tst_B, y_tst_R],['trn','val','tst','tst_ISUP', 'tst_B', 'tst_R']):
